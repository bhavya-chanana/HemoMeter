...
def plot_fiducials(s: pyPPG.PPG, fp: pyPPG.Fiducials, savefig=True, savingfolder='temp_dir', show_fig = True,
    ...
    if savefig:
        ...
        canvas.print_png((tmp_dir + 'result_%s_processed.png') % (s.name))
...
def save_data(savingformat: str, savingfolder: str, print_flag=True, s={}, fp=pd.DataFrame(), bm=pd.DataFrame()):
    ...
    file_names = {}
    file_name = (relative_path + tmp_dir + os.sep + temp_dirs[4] + os.sep + 'result_%s_processed.mat') % (s.name)
    ...
    if savingformat=="csv" or savingformat=="both":
        file_name = (relative_path+tmp_dir+os.sep+temp_dirs[0]+os.sep+s.name+'_'+'Fiducials_processed.csv')
        ...
        for key in BM_keys:
            file_name = (relative_path+tmp_dir+os.sep+temp_dirs[1]+os.sep+'%s_processed.csv')%(s.name+'_'+key)
            ...
            file_name = (relative_path+tmp_dir+os.sep+temp_dirs[2]+os.sep+'%s_processed.csv')%(s.name+'_'+key)
            file_names [key + '_stats_csv']= file_name
            bm.bm_stats[key].to_csv(file_name, index=True, header=True)

            file_name = (relative_path+tmp_dir+os.sep+temp_dirs[3]+os.sep+'%s_processed.csv')%(s.name+'_'+key)
    ...
    if savingformat=="mat"  or savingformat=="both":
        file_name = (relative_path+tmp_dir+os.sep+temp_dirs[0]+os.sep+s.name+'_'+'Fiducials_processed.mat')
        ...
        for key in BM_keys:
            ...
            file_name = (relative_path+tmp_dir+os.sep+temp_dirs[1]+os.sep+'%s_processed.mat')%(s.name+'_'+key)
            ...
            if len(bm.bm_stats)>0:
                file_name = (relative_path+tmp_dir+os.sep+temp_dirs[2]+os.sep+'%s_processed.mat')%(s.name+'_'+key)
            ...
            file_name = (relative_path+tmp_dir+os.sep+temp_dirs[3]+os.sep+'%s_processed.mat')%(s.name+'_'+key)
            ...
            file_name = (relative_path+tmp_dir+os.sep+temp_dirs[5]+os.sep+'%s_processed.mat')%(s.name+'_'+key)
